...
var landsat8data = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
...
var data_l8 = landsat8data.map(applyScaleFactors);
var l8LST = data_l8.filter(ee.Filter.date('2021-03-01', '2021-03-31')).select('ST_B10')
.mean().clip(geometry)
...
Map.addLayer(l8LST, visl8, 'Air Temperature Landsat8');
Map.addLayer(monthly_temp, {}, "Station Air Temperature");




var tempBands = l8LST.addBands(modisLST);
var label = "TEMP";
var bands = ["ST_B10", "LST_Day_1km"];
...
var RFR = ee.Classifier.smileRandomForest(1500).setOutputMode("REGRESSION")
  .train(trainSet.select(["ST_B10", "LST_Day_1km", "TEMP"]),
  'TEMP', ["ST_B10", "LST_Day_1km"])
